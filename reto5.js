...
function validarEmail(email) {
  try {
    ...
    const puntos = email.split('.')
    const arroba = puntos[puntos.length - 2].split('@')

    if (arroba[1].length < 2) return false
    if (puntos[puntos.length - 1] === '.') return false
...
for (let id in lista) {
  const row = lista[id].split(',')

  const validId = row[0] ? filterAlpha(row[0]) : false
  const validName = row[1] ? filterAlpha(row[1]) : false
  const validEmail = row[2] ? validarEmail(row[2]) : false
  const validAge = row[3] ? regex.test(row[3]) : true
  const validLocation = row[4] ? regex2.test(row[4]) : true
