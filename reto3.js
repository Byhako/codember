const policies = `8-10 r: ozrcdfnug
9-10 q: hvsazxrigf
1-9 j: bbqonxjt
...
lista = policies.split('\n')

const validas = []
const error = []

lista.forEach(element => {
  let [key, clave] = element.split(':')
  clave = clave.trim()

  const [rango, letra] = key.split(' ')
  const [start, end] = rango.split('-')

  const apariciones = clave.split('').filter(x => x === letra).length

  if (apariciones>=start && apariciones<=end) {
    validas.push(clave)
  } else {
    error.push(clave)
  }
